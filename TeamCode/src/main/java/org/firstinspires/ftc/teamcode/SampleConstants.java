package org.firstinspires.ftc.teamcode;

//import com.acmerobotics.dashboard.config.Config;
import com.qualcomm.robotcore.hardware.DcMotorSimple;

/*
 * Some additional constants for testing.
 */
//@Config
public class SampleConstants {
//    public static DcMotorSimple.Direction DIR = DcMotorSimple.Direction.FORWARD;
//    public static boolean FF_ENABLED = false;

    public static double KPX = 0.1;
    public static double KPY = 0.1;
    public static double KDX = 0.025;
    public static double KDY = 0.025;

    public static double KPH = 0.05;  // Scaled for direct error (no /10 hack)
    public static double KIH = 0;
    public static double KDH = 0.001;  // Scaled for direct error (no /10 hack)

    /*
    kpx = 0.16
    kpy = 0.16
    kdx = 0.14
    kdy = 0.14
     */
    // remove the camera volt issues by lowering the computation amoutn from the cameras basedo ff of their pipelines, do config stuff
}